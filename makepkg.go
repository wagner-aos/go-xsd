...
var (
	PkgGen = &pkgGen{
		BaseCodePath:             udevgo.GopathSrcGithub("wagner-aos", "go-xsd-pkg"),
		BasePath:                 "github.com/wagner-aos/go-xsd-pkg",
...
func newPkgBag(schema *Schema) (bag *PkgBag) {
	...
	bag.imports, bag.impsUsed, bag.lines = map[string]string{}, map[string]bool{}, []string{"//\tAuto-generated by the \"go-xsd\" package located at:", "//\t\tgithub.com/wagner-aos/go-xsd", "//\tComments on types and fields (if any) are from the XSD file located at:", "//\t\t" + bag.Schema.loadUri, "package go_" + bag.safeName(ustr.Replace(path.Base(bag.Schema.RootSchema([]string{bag.Schema.loadUri}).loadUri), map[string]string{"xsd": "", "schema": ""})), ""}
	bag.imports[bag.impName] = "github.com/wagner-aos/go-xsd/types"
